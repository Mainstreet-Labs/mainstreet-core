 ...
 */
contract MainstreetMinterTest is BaseSetup, IErrors {
    ...
    function testInitState() public {
        assertNotEq(msUSDToken.feeSilo(), address(0));
    ...
    function testMinterMintAfterRebaseFuzzing(uint256 index) public {
        ...
        uint256 foreshadowTS = (((preTotalSupply * 1e18) / msUSDToken.rebaseIndex()) * index) / 1e18;

        uint256 delta = (preTotalSupply * index / 1e18) - preTotalSupply;

        vm.prank(rebaseManager);
        msUSDToken.rebaseWithDelta(delta);

        assertApproxEqAbs(msUSDToken.totalSupply(), foreshadowTS, 100);

        uint256 amount = 10 ether;
        deal(address(FRAX), alice, amount);
    ...
    function testMinterRequestTokensAfterRebaseNoFuzz() public {
        // Config
        ...
        assertEq(FRAX.balanceOf(alice), 0);
        assertEq(FRAX.balanceOf(address(msMinter)), amount);
        assertApproxEqAbs(msUSDToken.balanceOf(alice), amount, 0);
        ...
        // update rebaseIndex on msUSD

        vm.prank(rebaseManager);
        msUSDToken.rebaseWithDelta(delta);

        assertApproxEqAbs(msUSDToken.totalSupply(), foreshadowTS, 5);
        uint256 newBal = (amount * msUSDToken.rebaseIndex()) / 1e18;
        assertGt(newBal, amount);
        assertApproxEqAbs(msUSDToken.balanceOf(alice), newBal, 0);
        deal(address(FRAX), address(msMinter), newBal);

        assertEq(msMinter.quoteRedeem(address(FRAX), alice, newBal), newBal);

        // Alice executes requestTokens

        vm.startPrank(alice);
        msUSDToken.approve(address(msMinter), newBal);
        msMinter.requestTokens(address(FRAX), newBal);
        vm.stopPrank();
    ...
    function testMinterRequestTokensAfterRebaseFuzzing(uint256 index) public {
    ...
    function testMinterClaimAfterRebaseNoFuzz() public {
        // Config
        ...
        assertEq(msUSDToken.balanceOf(alice), 0);
        assertEq(FRAX.balanceOf(alice), newBal);
        assertEq(FRAX.balanceOf(address(msMinter)), 0);
